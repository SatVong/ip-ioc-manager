-- Инициализация базы данных IP/IOC Manager
...
CREATE TABLE IF NOT EXISTS ioc_records (
    ...
    from_source VARCHAR(64),
    letter VARCHAR(24),
    indicator VARCHAR(128),
    encoding VARCHAR(10),
    status_opentip VARCHAR(64),
    status_virustotal VARCHAR(64),
    note_in VARCHAR(128),
    date_in VARCHAR(16),
    who_in VARCHAR(100),
    note_out VARCHAR(128),
...
-- Таблица белых IP
CREATE TABLE IF NOT EXISTS white_ip_records (
    id SERIAL PRIMARY KEY,
    mses INTEGER[] DEFAULT '{}',
    date VARCHAR(10),
    from_source VARCHAR(64),
    ...
    date_in VARCHAR(16),
    who_in VARCHAR(100),
    note_out VARCHAR(128) DEFAULT '-',
    date_out VARCHAR(16) DEFAULT '-',
    who_out VARCHAR(100) DEFAULT '-',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX IF NOT EXISTS idx_ioc_records_indicator ON ioc_records(indicator);
CREATE INDEX IF NOT EXISTS idx_white_ip_records_ip ON white_ip_records(ip);
CREATE INDEX IF NOT EXISTS idx_white_ip_records_date ON white_ip_records(date);
