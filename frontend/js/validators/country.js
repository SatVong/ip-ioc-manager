...
export function validateCountry(input) {
    ...
    const countryUpper = value.toUpperCase();
    if (!validCountries.has(countryUpper)) {
        input.classList.add('error');
        input.classList.remove('valid');
        return { valid: false, error: `Страна "${value}" не входит в список допустимых стран. Если страна не известна внеси XX` };
    }
