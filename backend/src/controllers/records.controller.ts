...
const IP_SEARCH_COLUMNS = ['ip', 'from_source', 'domain', 'owner', 'soib_infr', 'note_in'];
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO ip_records 
       ...
       RETURNING *`,
      [data.mses, data.date, data.from_source, data.letter, data.domain, data.ip,
...
export async function updateRecord(req: Request, res: Response): Promise<void> {
  try {
    ...
    const result = await pool.query(
      ...
      [data.mses, data.date, data.from_source, data.letter, data.domain, data.ip,
       data.country, data.owner, data.mse_method, data.note_in, data.soib_infr,
       data.date_in, data.who_in, data.note_out, data.date_out, data.who_out, id]
    );
